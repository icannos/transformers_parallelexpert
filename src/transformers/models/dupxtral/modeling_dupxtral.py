...
class DuxtralAttention(nn.Module):
    ...
    def __init__(self, config: DupxtralConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
MIXTRAL_ATTENTION_CLASSES = {
    "eager": DuxtralAttention,
    "flash_attention_2": DuxtralFlashAttention2,
    "sdpa": DuxtralSdpaAttention,
}
...
class DupxtralSparseMoeBlock(nn.Module):
    ...
    def __init__(self, config: DupxtralConfig, layer_idx: int):
        ...
        # make a tensor of shape (num_experts, num_experts) to map the expert indices
        initial_number_of_experts = len(config.experts_duplicate[layer_idx])

        self.expert_pair_mapping = torch.zeros(
            (
                initial_number_of_experts,
                initial_number_of_experts,
...
class DupxtralDecoderLayer(nn.Module):
    def __init__(self, config: DupxtralConfig, layer_idx: int):
        ...
        self.input_layernorm = DupxtralRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
        self.post_attention_layernorm = DupxtralRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps
        )
...
class DupxtralModel(DupxtralPreTrainedModel):
    ...
    def __init__(self, config: DupxtralConfig):
        ...
        self.layers = nn.ModuleList(
            [
                DupxtralDecoderLayer(config, layer_idx)
