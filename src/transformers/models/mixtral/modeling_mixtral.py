...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.forward == "base":
            self.forward = self.base_forward
        elif config.forward == "parallel":
            self.forward = self.forward_with_parallelism
        ...
        with record_function("Expert Computation"):
            for expert_idx in range(self.num_experts):
                with record_function(f"Expert Computation {expert_idx}"):
                    ...
                    current_hidden_states *= routing_weights[top_x, idx, None].to(
                        current_hidden_states.device
                    )
                    ...
                    final_hidden_states.index_add_(
                        0,
                        top_x,
                        current_hidden_states.to(hidden_states.dtype).to(
    ...
    def forward_with_parallelism(self, hidden_states: torch.Tensor) -> torch.Tensor:
        """ """
        batch_size, sequence_length, hidden_dim = hidden_states.shape
        if self.training and self.jitter_noise > 0:
            hidden_states *= torch.empty_like(hidden_states).uniform_(
